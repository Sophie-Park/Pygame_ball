...
# 폰트 정의
game_font = pygame.font.Font(None, 40) # None - 디폴트 폰트 사용 # 폰트 객체 생성 (폰트, 크기)
...
# 타이머 집어 넣기
# 경과 시간 계산(ms)
    elapsed_time = (pygame.time.get_ticks()  - start_ticks) / 1000 # 밀리세컨드이기에 1000으로 나누어서 초로 환산하기 위하여
    
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255,255,255)) # int - 정수로 바꾸려고
    # render(출력할 글자, True, 글자색상)
    screen.blit(timer, (10,10)) # 타이머를 표시하려고
...
# 잠시 대기 - 종료 직전
pygame.time.delay(2000) # 2000 = 2초 # 2초정도 대기
